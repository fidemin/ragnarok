...
from _old_src.main.core.learning_rate import InverseSqrt
from _old_src.main.core import NeuralNet
from _old_src.main.core import Adam
from _old_src.main.data.sum_data_creator import (
    create_sum_examples,
    convert_sum_examples_to_str,
)
from _old_src.main.language.layer import GroupedSoftmaxWithLoss
from _old_src.main.language.seq2seq import Seq2Seq, AttentionSeq2Seq
from _old_src.main.language.util import WordIdConverter, convert_to_one_hot_encoding


def load_data(number_of_examples, min_int, max_int, proportion_of_test=0.1):
    seed = datetime.now().timestamp()
    sum_examples_original = create_sum_examples(
        number_of_examples, min_int, max_int, seed=seed
    )
    ...
    left_sum_id_examples = [
        [wi_converter.word_to_id(char_) for char_ in char_list]
        for char_list in left_sum_char_examples
    ]
    right_sum_id_examples = [
        [wi_converter.word_to_id(char_) for char_ in char_list]
        for char_list in right_sum_char_examples
    ]
    ...
    training_X, test_X = np.split(
        left_sum_id_examples, [int(length * (1 - proportion_of_test))]
    )
    training_y, test_y = np.split(
        right_sum_id_examples, [int(length * (1 - proportion_of_test))]
    )
    return training_X, training_y, test_X, test_y, wi_converter


def convert_decoder_input_to_expected_output(
    decoder_input: np.ndarray, max_word_id: int, one_hot=True
):
    output = np.delete(decoder_input, 0, axis=1)
    output = np.insert(output, output.shape[1], values=word_id_blank, axis=1)

    if one_hot:
        output = convert_to_one_hot_encoding(output, max_word_id)
    return output
...
if __name__ == "__main__":
    ...
    print(
        "train X: {}, train y: {}, test X: {}, text y: {}".format(
    ...
    train_t = convert_decoder_input_to_expected_output(
        train_y, wi_converter.max_id(), one_hot=True
    )
    test_t_no_one_hot = convert_decoder_input_to_expected_output(
        test_y, wi_converter.max_id(), one_hot=False
    )
    ...
    seq2seq = AttentionSeq2Seq(voca_size, wordvec_size, hidden_size)
    loss_layer = GroupedSoftmaxWithLoss()

    net = NeuralNet(
        [seq2seq], loss_layer=loss_layer, optimizer=Adam(lr=InverseSqrt(0.01))
    )
    ...
    for epoch in range(1, max_epoch + 1):
        ...
        for iter_ in range(iter_size):
            ...
            net.backward()
            net.optimize(grad_max_norm=5.0, epoch=epoch)
