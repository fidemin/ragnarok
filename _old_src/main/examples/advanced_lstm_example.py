...
from _old_src.main.core.layer import Dropout
...
if __name__ == "__main__":
    ...
    xs = xs[:data_size]
    print(
        "original data size: {}, truncated data size:, {}, voca_size: {}".format(
            original_data_size, data_size, voca_size
        )
    )

    W_emb = np.random.randn(voca_size, wordvec_size) * 0.01
    Wx_lstm1 = np.random.randn(wordvec_size, 4 * hidden_size) * (
        1 / np.sqrt(wordvec_size)
    )
    Wh_lstm1 = np.random.randn(hidden_size, 4 * hidden_size) * (
        1 / np.sqrt(hidden_size)
    )
    b_lstm1 = np.zeros((1, 4 * hidden_size))

    Wx_lstm2 = np.random.randn(hidden_size, 4 * hidden_size) * (
        1 / np.sqrt(hidden_size)
    )
    Wh_lstm2 = np.random.randn(hidden_size, 4 * hidden_size) * (
        1 / np.sqrt(hidden_size)
    )
