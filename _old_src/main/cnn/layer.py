...
from _old_src.main.cnn.util import img2col, fil2col, col2fil, col2img
from _old_src.main.core.layer import Layer
from _old_src.main.core.optimizer import Optimizer


class Convolution(Layer):
    def __init__(
        self, F: np.ndarray, b: np.ndarray, updater: Optimizer, stride=1, padding=0
    ):
    ...
    @classmethod
    def from_sizes(
        cls, F_shape, updater: Optimizer, stride=1, padding=0, weight_init=0.01
    ):
        # F_shape: (FN, FC, FH, FW)
    ...
    def forward(self, x: np.ndarray, **kwargs) -> np.ndarray:
        ...
        if C != FC:
            ...
            raise Exception(
                "Channel of X and filter should be same. C: {0}, FC: {1}".format(C, FC)
            )
