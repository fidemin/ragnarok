...
from _old_src.main.core.learning_rate import LearningRate
...
class SGD(Optimizer):
    ...
    def optimize(
        self, params: list[np.ndarray], grads: list[np.ndarray], epoch=1
    ) -> list[np.ndarray]:
        lr = self._lr
...
class Adam(Optimizer):
    ...
    def optimize(
        ...
        for i in range(len(params)):
            self._momentum1[i] = (
                self._beta1 * self._momentum1[i] + (1 - self._beta1) * grads[i]
            )
            momentum1_corr = self._momentum1[i] / (1 - (self._beta1**self._iter))
            self._momentum2[i] = self._beta2 * self._momentum2[i] + (
                1 - self._beta2
            ) * np.power(grads[i], 2)
