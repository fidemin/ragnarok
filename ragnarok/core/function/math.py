...
class Square(Function):
    ...
    def _validate_tensors(self, *tensors: Tensor):
        ...
        if num_tensors == 0 or num_tensors > 1:
...
class Pow(Function):
    ...
    def _validate_tensors(self, *tensors: Tensor, **kwargs):
        if "power" not in kwargs:
            raise FunctionTensorError("power is required for Pow function.")
        power = kwargs["power"]
        if not isinstance(power, (int, float)):
            raise FunctionTensorError("power should be int or float for Pow function.")
        var_length = len(tensors)
        if var_length != 1:
            raise FunctionTensorError(
